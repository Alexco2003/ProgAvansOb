package org.example.lab8.src.main.java.csv;
...
public class CsvPrintWriter {
    ...
    private static List<String[]> buildSampleData() {
        ...
        sampleDataLines.add(new String[]
                { "Marcel", "Ionescu", "40", "email_marcel@abc.com" });
        sampleDataLines.add(new String[]
                { "Sorin", "Popescu", "20", "email_sorin@def.com" });
        return sampleDataLines;
    }

    private static void writeUsingPrintWriter(List<String[]> dataLines) {
        try (PrintWriter pw = new PrintWriter(new File("./src/main/java/org/example/lab8/csv_PrintWriter_output.csv"))) {
